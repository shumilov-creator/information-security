    ...
    case WM_COMMAND: {
        ...
        case ID_BTN_HELP: {
            wstring help =
                ...
                L"   • В верхней части окна указано имя активного пользователя; все операции выполняются от его имени.\n"
                L"   • Нажмите «🔄 Сменить пользователя», чтобы войти под другим или создать нового: пароль защищает его ключи и файлы.\n"
                L"   • Для каждого пользователя автоматически создаются отдельные папки автосохранений и обмена.\n\n"
                L"2. Ключи шифрования\n"
                L"   • Менеджер ключей (меню «Сервис → Менеджер ключей») позволяет хранить несколько ключей с метками.\n"
                L"   • Отметьте ⭐ текущий ключ — он будет использоваться для шифрования и обмена.\n"
                L"   • Если ключей ещё нет, создайте новый и укажите понятную метку, чтобы не перепутать.\n\n"
                L"3. Шифрование/расшифрование\n"
                L"   • Введите текст слева или загрузите его через «Файл → Открыть...».\n"
                L"   • Выберите режим (для совместимости и стойкости рекомендуется CBC).\n"
                L"   • Нажмите «🔒 Зашифровать» или «🔓 Расшифровать»; кнопка «▶ Расшифровать последнее» использует последний контейнер.\n"
                L"   • Результат появляется справа и автосохраняется в «Выходные файлы\\\\<пользователь>». При необходимости сохраните вручную через «Файл → Сохранить».\n\n"
                L"4. Обмен\n"
                L"   • Открывается через меню «Сервис → Обмен файлами».\n"
                L"   • Использует левое поле основного окна как текст для отправки; полученные сообщения доступны в отдельной папке обмена.\n"
                L"   • Перед отправкой убедитесь, что у выбранного получателя есть актуальный ключ.\n\n"
